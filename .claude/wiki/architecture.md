...
**Audit-log contract (bash producer, Node consumer):** The dashboard reads the **Microworld audit log** at `.claude/microworld-audit.log` (gitignored), written by the `microworld-rerun.sh` **Reporter** hook (`hooks/scripts/microworld-rerun.sh:19-26`) on every `PostToolUse` (`Edit|Write`) invocation to track which bundles have run most recently and whether they passed or failed. The log is not JSON — each line is space-separated `key=value` text: `<ts> unit=<slug> result=<pass|fail|timeout|error> file=<path> [reason=<...>]` (built at `microworld-rerun.sh:22-24`). This is a contract-tested interface — the hook's own comment says "Do not change the separator or format," guarded by `tests/microworld/microworld-audit-contract.test.js`. The Node consumer, `parseAuditLog` in `bin/microworld-dashboard/audit-log.js`, is invoked **per request** from inside `discover()` (`bin/microworld-dashboard/discover.js`), never on startup. `fs.watch` is used in `bin/microworld-dashboard/server.js`, but on the `microworlds/` bundle directory, not the audit log — and that specific callback is an explicit no-op ("Bundle structure changed; next GET request will pick up the change"). The server therefore never pushes. Keeping status fresh is the dashboard's browser client's job instead: after the initial render, `index.html:570` registers `setInterval(pollBundles, 5000)`, so the client re-issues `GET /api/bundles` every 5s and re-renders the left rail's audit-log-derived status indicator (`renderLeftRail`, `index.html:124`) with no user action — a polling loop, not a per-request pull.
