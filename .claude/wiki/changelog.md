...
## 2026-09-12 (orch-rulings-followup-1 post-PASS advisory resolution — scribe duty)
- **Completed unit orch-rulings-followup-1 (ad-hoc follow-up, no issue, PASS 2026-09-11) — scribe resolved reviewer's final advisory note on protocol vocabulary.**
  Reviewer flagged `agents/orchestrator.md:449`'s newly introduced phrase "natural turn boundaries" as a near-neighbour term to the existing protocol vocabulary's "turn-end", recommending both terms be pinned down with a glossary entry rather than left as an implicit synonym pair. Added `CONTEXT.md` glossary entry **turn-end** / **natural turn boundaries** disambiguating the two concepts: **turn-end** is the specific protocol event marking the conclusion of a dispatched turn (where control returns to a caller, subject of the **Terminal status line** section in the **Shared persona protocol**); **natural turn boundaries** is broader — natural transition points in ongoing work (including turn-ends, but also other natural breaking points like task completion). Every turn-end is a natural turn boundary; the converse does not hold. Entry documents the orchestrator's usage: "Space polls out across your natural turn boundaries (e.g. after other work, or the next time you're about to act) rather than checking in a tight loop." **Affected files:** 1 (CONTEXT.md — one glossary entry). No ADR needed (vocabulary clarification only). No tracker issue (ad-hoc follow-up).

## 2026-09-12 (orch-rulings-wait-heuristic unit closure — scribe duty)
- **Completed unit orch-rulings-wait-heuristic (ad-hoc doc addition, no issue) — scribe recorded institutional knowledge after reviewer PASS.**
  This unit added a **Rulings ledger** section to `agents/orchestrator.md`, documenting a new advisory-only log at `.claude/orchestrator-rulings.log` where the orchestrator records self-authorized judgment calls (e.g., tier overrides). Scribe recorded three institutional updates to `CONTEXT.md`: (1) added **Rulings ledger** glossary entry clarifying it is a fifth member of the Log domain, explicitly distinct from the sealed audit-log family (review-audit.log, wip-audit.log, microworld-audit.log, dispatch-audit.log); (2) updated existing **Log domain** and **state-artifact species** entries to specify "sealed audit logs" instead of "the four audit logs", now disambiguated via forward reference to the new Rulings ledger entry; (3) added **ruling / rulings disambiguation** glossary entry because "ruling" now refers to two distinct actors — human operators (via escalation) and the orchestrator (via the new log). The disambiguation notes both senses and clarifies the recording mechanisms. **Affected files:** 1 (CONTEXT.md — three glossary entries: one new, two enhanced with clarifications). Also updated `.claude/wiki/changelog.md` (this entry). No ADR needed (institutional glossary work only). Fixed commit `4d709c1`. No tracker issue (ad-hoc doc addition dispatched directly).
