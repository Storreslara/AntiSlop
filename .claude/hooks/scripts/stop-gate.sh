...
if [ "$hook_event" = "SubagentStop" ] && [ "$(identity_persona_name "$agent_type")" = "reviewer" ]; then
  if persona_matches_grant "$agent_type" reviewer; then
    ...
    if [ "${JOIN_STAMP_COUNT:-0}" -eq 0 ]; then
        ...
        if [ "$mcc_mode" != off ]; then
          ...
          if [ "$mcc_state" = mismatch ]; then
            ...
            if [ "$mcc_mode" = block ]; then
              echo "Remediation: correct the commit: field in .claude/reviewed/${unit}.pass to name the unit's own final commit, then re-run." >&2
              exit 2
            fi
          fi
        fi
      ...
      echo "  printf 'PASS <unit-id> %s commit: %s criteria: bash tests/validate.sh\\n' '$(date -u +%Y-%m-%dT%H:%M:%SZ)' '<the unit's own final commit, not HEAD>' > .claude/reviewed/<unit-id>.pass" >&2
