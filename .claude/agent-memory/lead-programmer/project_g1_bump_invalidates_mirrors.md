...
- **Never hand-edit a `.claude/agents/*.md` mirror.** Edit only the
  `agents/*.md` source, then run `node bin/cli.js --update` to let it copy
  the mirror over — that's the only fix that avoids the stamp gap below, so
  treat "match this mirror to my source edit by hand" as always wrong.
  Footnote (recovery, if it already happened): if you hand-add the same
  content to both `agents/X.md` and `.claude/agents/X.md` yourself before
  running `--update`, the recorded hash is stale (doesn't match your
  hand-edit) but the byte content now exactly equals a fresh render, so
  `runUpdate`'s per-file branch (`bin/cli.js` ~line 1400) takes the silent
  "hash healed (content unchanged, hash was stale)" path — it fixes
  `fileHashes` but does NOT touch the `<!-- antislop v... -->` stamp, leaving
  that one mirror at the old version while its 9 siblings get bumped
  normally. Recover by running `node bin/cli.js --update --force-render` a
