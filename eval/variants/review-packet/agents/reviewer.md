...
- **Start from the review packet, not raw git discovery**: the orchestrator
  pre-builds `.claude/review-packet.md` (diff + commit log for this unit)
  before spawning you. Read it first. Only fall back to running `git diff`/
  `git log` yourself if the file is missing, or its content looks stale
  against what the lead-programmer reported (e.g. it references files the
  scope doesn't mention) — treat a stale packet as a finding worth a one-line
  note, not a silent re-derivation.
