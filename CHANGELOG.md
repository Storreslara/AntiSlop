...
**New `version-stamp-check.sh`: mechanizes constitution P3's version-stamp-discipline check (version-stamp-guard-1).** A reviewer-invoked helper, matching the `heavy-trigger.sh`/`reviewer-tier.sh` precedent — takes a `<commit-range>` argument, is deliberately **not** registered in `hooks/hooks.json`, and always exits 0 while printing `version-stamp-check: <ok|violation|unknown> touched: <yes|no|-> old: <ver|-> new: <ver|->`. If the range's diff touches `agents/*.md` or anything under `templates/`, `.claude-plugin/plugin.json`'s `version` must differ between the range's two ends, or the verdict is `violation`. A standalone script (rather than folding the check into `tests/validate.sh`) was chosen specifically because CI's shallow clone (`actions/checkout@v4`'s default `fetch-depth: 1`, per `.github/workflows/validate.yml`) makes a self-deriving `HEAD~1`-based check unreliable there; an explicit range supplied by the reviewer, who already has the unit's commits locally, sidesteps that entirely. Confined to `hooks/scripts/`, `tests/`, and `docs/`, so per constitution P3's own literal scope (`agents/*.md`, `templates/`) this unit needs no version bump itself — confirmed against CHANGELOG precedent (`harness-integrity-gate.sh`, `dispatch-hygiene.sh`, and other hooks/scripts-only additions land without a bump).
