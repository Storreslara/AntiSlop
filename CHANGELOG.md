...
- **`adapters/codex/agents-md-fragment.md`**: the spec's dispatch packet listed `agents/orchestrator.md` as the third location, but that file was verified (full read, all 547 lines) to carry no such claim at all — it never discusses why the escalation packet avoids `.claude/reviewed/`, only that the reviewer already placed it under `.claude/human-review/`. The actual third location, found by a repo-wide grep, is this hand-authored Codex adapter fragment (not auto-regenerated by `--update`, since no `.codex/` mirror exists in this repo), which carried the identical false sentence. Corrected the same way, matching the fragment's plain-ASCII-hyphen style.
