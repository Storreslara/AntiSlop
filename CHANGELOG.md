...
**0.31.82 — bump version and changelog for the human-confirmation branch merge gate (hcb-release, LAST unit of docs/plans/2026-09-23-harness-integrity-gate-human-confirmation.md).** Closes out the 9-unit slice that added a human-confirmation branch to `hooks/scripts/harness-integrity-gate.sh`: instead of the gate outright denying every write, five paths now route through this branch — the harness persona-selection config file (Set A's one ask-eligible member) and Set B's four registration-surface literals, `hooks/hooks.json`, `.claude/settings.json`, `hooks/scripts/harness-integrity-gate.sh`, and `.claude/hooks/scripts/harness-integrity-gate.sh` (the gate's mirror is now Set B's fourth literal). The two tiers ask on different `permission_mode` sets: Set A asks in `default`, `plan`, `acceptEdits`, and `auto`; Set B asks in `default`, `plan`, and `auto` only, excluding `acceptEdits`. Every other mode — including an absent, empty, or unrecognised `permission_mode` — denies, for both sets. The gate never gains an `allow` branch, and never emits `ask` when a subagent identity is present, for either set. A new `PostToolUse (Edit|Write)` completion record logs that a human-confirmable write completed; its `hook_event_name` check runs as the first action in the script, ahead of the deny path, since a PostToolUse payload carries no `.tool_input.command` and would otherwise fall through to a false deny. The Set B `asked`/`completed` audit pair is ambiguous by design — an approved Set B write can suppress its own completion record — and that ambiguity is reconciled out of band, by the registration-presence assertion, not by the audit pair itself. The Bash branch is unchanged: it keeps today's unconditional deny, as do the four audit logs (`review`, `dispatch`, `microworld`, `wip`) and their `.seal` sidecars. Step 5's operator-side work removed the `.claude/settings.local.json` `permissions.allow` entry for the persona-selection config and recorded a live characterization of all 6 documented `permission_mode` values plus a headless run in `docs/experiments/2026-09-23-probe-permission-mode-ask.md`, confirming `ask` still renders a prompt over an `allow` entry — the precondition Set B's half of this slice shipped on.

### Changed
- **`package.json`**, **`.claude-plugin/plugin.json`**: version bump 0.31.81 → 0.31.82.
- **`.claude/agents/*.md`** (all ten), **`.claude/persona-config.json`**, **`.claude/persona-protocol.md`**, **`.claude/persona-protocol-slim.md`**, **`.claude/protocol-digest.md`**: regenerated via `node bin/cli.js --update` to pick up the version stamp only. `.claude/hooks/scripts/**` was already byte-identical to `hooks/scripts/**` from earlier units in this slice, so `--update` made no change there.

**0.31.81 — fix two non-blocking review findings in the Effort-tier policy prose (orchestrator-effort-policy-prose-fix, follow-up to cost-governance-step5-effort-policy).** `agents/orchestrator.md`'s "Effort-tier policy" subsection no longer contradicts itself: the "may raise" paragraph now states explicitly that raising means **editing a persona's definition file** (`agents/<persona>.md`'s frontmatter — a guarded change, not a per-dispatch flag), not a live per-dispatch override, matching the "not settable per dispatch" paragraph above it and `docs/adr/0033-effort-tiers-frontmatter-only-override.md:51-54`. The stated value list is corrected from `low|medium|high` to `low|medium|high|xhigh|max`, or an integer, matching `CONTEXT.md`'s "effort override / effort tier" entry and `tests/effort-tier-consistency.test.js`. Also adds the missing explorer half-case (declared `low` must never silently rise under a hot ambient session) alongside the existing reviewer half-case, per ADR-0033:29-32.

### Changed
- **`agents/orchestrator.md`**: Effort-tier policy prose fixes (contradiction, value list, explorer half-case).
- **`.claude-plugin/plugin.json`**: version bump 0.31.80 → 0.31.81.
- **`package.json`**: version bump 0.31.80 → 0.31.81.
- **`.claude/agents/orchestrator.md`**: regenerated via `node bin/cli.js --update` to pick up the version stamp and corrected prose.

**0.31.80 — document the effort-tier policy and resolve both Codex `model_reasoning_effort` TODOs (cost-governance-step5-effort-policy, Step 5 of the cost-governance-output-cap-and-effort-tiers plan).** `agents/orchestrator.md` gains a new "Effort-tier policy" subsection, adjacent to "Reviewer gate model selection": effort is declared per persona in frontmatter and has no dispatch-time knob (the `Agent` tool's schema exposes only `description`, `prompt`, `subagent_type`, `model`, `isolation`); the one-way rule runs the same direction as the `sonnet` → `opus` model-tier escalation — judgment may raise a persona's effort above its declared value, never lower it; and `reviewer` is a hard exclusion whose `high` effort is pinned (an override, not a floor — see `docs/adr/0033-effort-tiers-frontmatter-only-override.md`), mirroring the file's existing "Fable is never valid on the gate" pattern. Both long-deferred Codex TODOs are resolved with Step 4's decided values: `adapters/codex/agents/reviewer.toml` now sets `model_reasoning_effort = "high"` and `adapters/codex/agents/explorer.toml` sets `"low"`. Cursor's equivalent ports gain the matching `model: inherit[effort=high]` / `model: inherit[effort=low]` annotation, per `docs/specs/codex-cursor-plugin.md` row 8's documented `[effort=...]` suffix syntax — UNVERIFIED against a real Cursor build, same caveat as the pre-existing `model: inherit` model-id mapping note in those same files. `tests/adapter-protocol-parity.test.js` gains a standalone probe (not a `codexMap`/`cursorMap` row, since the new orchestrator clause has no `templates/persona-protocol.md` mirror to key off of) asserting the orchestrator heading, its never-lower and hard-exclusion language, and all four resolved adapter values.

### Added
- **`tests/adapter-protocol-parity.test.js`**: new standalone probe covering the orchestrator's Effort-tier policy clause and the four resolved Codex/Cursor adapter values.

### Changed
- **`agents/orchestrator.md`**: new "Effort-tier policy" subsection.
- **`adapters/codex/agents/reviewer.toml`**: resolved TODO, `model_reasoning_effort = "high"`.
- **`adapters/codex/agents/explorer.toml`**: resolved TODO, `model_reasoning_effort = "low"`.
- **`adapters/cursor/agents/reviewer.md`**: `model: inherit` → `model: inherit[effort=high]`, plus a port-note line.
- **`adapters/cursor/agents/explorer.md`**: `model: inherit` → `model: inherit[effort=low]`, plus a port-note line.
- **`.claude-plugin/plugin.json`**: version bump 0.31.79 → 0.31.80.
- **`package.json`**: version bump 0.31.79 → 0.31.80.
- **`.claude/agents/*.md`** (all ten), **`.claude/persona-protocol.md`**, **`.claude/persona-protocol-slim.md`**, **`.claude/protocol-digest.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update` to pick up the version stamp (content unchanged for the mirrors other than `orchestrator.md`, which carries the new clause).

**0.31.79 — declare `effort:` frontmatter tiers on three Claude personas (cost-governance-step4-effort-tiers, Step 4 of the cost-governance-output-cap-and-effort-tiers plan).** `explorer` now declares `effort: low` (mechanical traversal), `task-master` declares `effort: medium` (mechanical slicing against an already-finalized spec), and `reviewer` declares `effort: high` as an override that supersedes ambient session effort — preventing both the inheritance of weaker effort from the main session and capping unintended elevation to stronger tiers. `milestone-auditor` deliberately carries no `effort:` key at all, per the user's settled 2026-09-23 decision (its adversarial judgment is per-persona, not per-phase, so its one mechanical phase can't be priced separately) — `tests/effort-tier-consistency.test.js` asserts this absence positively, not just by omission. The same test asserts every declared value is a member of `["low","medium","high","xhigh","max"]` or an integer, and pins `reviewer`'s `high` with a dedicated mutation-proof assertion (changing it to `low` fails that assertion specifically, not just a generic membership check). Empirical pin for the effort-inheritance claim: `strings`-scanned all four locally installed Claude Code binaries (2.1.277, 2.1.278, 2.1.280, 2.1.281 — the only versions present under `~/.local/share/claude/versions/`); all four contain the identical validator string `has invalid effort '<value>'. Valid options: <list> or an integer` and the literal valid-set array `["low","medium","high","xhigh","max"]`, confirming the key is recognized and the chosen tier values are all valid. A live dispatch of `explorer` under its new `effort: low` frontmatter also completed a trivial subagent turn with no dispatch failure and no "has invalid effort" warning — together this confirms the key is *declared* and *loads* correctly, though R7's stronger claim (that effort is silently inherited end-to-end when absent) remains unproven past that point. `effort:`'s true introduction version could not be established: only 2.1.277-2.1.281 are present locally, so there is nothing earlier to bisect against — same limitation as the sibling `bashOutputMaxChars` and `experimental.cacheTtl` keys (see the 0.31.76 and 0.31.77 entries). The plugin's declared compatibility floor (`>=2.1.248`) is therefore left unchanged rather than guessed at; a project on an older, untested version degrades silently (an unrecognized frontmatter key is dropped, not fatal), per Claude Code's own strict-schema behavior confirmed for other keys in this same file's history.

### Added
- **`tests/effort-tier-consistency.test.js`** (new): asserts source+mirror `effort:` parity for `explorer`/`task-master`/`reviewer`, valid-tier-or-integer membership repo-wide (scoped to `agents/` and `.claude/agents/`, the only dirs using this frontmatter shape), a mutation-proof pin on `reviewer`'s `high`, and a positive assertion that `milestone-auditor` and its mirror both lack `effort:`.

### Changed
- **`agents/explorer.md`**: new top-level `effort: low` frontmatter key.
- **`agents/task-master.md`**: new top-level `effort: medium` frontmatter key, placed immediately after the `experimental:`/`cacheTtl: 1h` block.
- **`agents/reviewer.md`**: new top-level `effort: high` frontmatter key, same placement.
- **`tests/validate.sh`**: registers the new suite.
- **`.claude-plugin/plugin.json`**: version bump 0.31.78 → 0.31.79.
- **`package.json`**: version bump 0.31.78 → 0.31.79.
- **`.claude/agents/explorer.md`**, **`.claude/agents/task-master.md`**, **`.claude/agents/reviewer.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update` to pick up the new frontmatter key and the version stamp.
...
### Changed
- **`agents/task-master.md`**, **`agents/reviewer.md`**: frontmatter now reads
  ```
  experimental:
    cacheTtl: 1h
  ```
  immediately below `model:`.
- **`.claude-plugin/plugin.json`**: version bump 0.31.75 → 0.31.76; compatibility floor raised 2.1.178 → 2.1.248.
- **`package.json`**: version bump 0.31.75 → 0.31.76.
- **`.claude/agents/task-master.md`**, **`.claude/agents/reviewer.md`**: `experimental.cacheTtl: 1h` propagated to the project-side mirrors.
- **`.claude/agents/*.md`** (all ten), **`.claude/persona-config.json`**, **`.claude/persona-protocol.md`**, **`.claude/persona-protocol-slim.md`**, **`.claude/protocol-digest.md`**: regenerated via `node bin/cli.js --update` to pick up the version stamp.

**New `version-stamp-check.sh`: mechanizes constitution P3's version-stamp-discipline check (version-stamp-guard-1, enhanced version-stamp-check-roast-1).** A reviewer-invoked helper, matching the `heavy-trigger.sh`/`reviewer-tier.sh` precedent — takes a `<commit-range>` argument, is deliberately **not** registered in `hooks/hooks.json`, and always exits 0 while printing `version-stamp-check: <ok|violation|unknown> touched: <yes|no|-> old: <ver|-> new: <ver|->`. The script implements **per-commit semantics**: for every commit within the range that touches `agents/*.md` or anything under `templates/`, `.claude-plugin/plugin.json`'s `version` must have changed compared to that commit's immediate parent, or the verdict is `violation`. This catches version-bump violations that endpoint-only comparison could mask (e.g., a later unrelated commit bumps the version, hiding an earlier omission). The endpoint `old`/`new` fields remain informational only. A standalone script (rather than folding the check into `tests/validate.sh`) was chosen specifically because CI's shallow clone (`actions/checkout@v4`'s default `fetch-depth: 1`, per `.github/workflows/validate.yml`) makes a self-deriving `HEAD~1`-based check unreliable there; an explicit range supplied by the reviewer, who already has the unit's commits locally, sidesteps that entirely. Confined to `hooks/scripts/`, `tests/`, and `docs/`, so per constitution P3's own literal scope (`agents/*.md`, `templates/`) this unit needs no version bump itself — confirmed against CHANGELOG precedent (`harness-integrity-gate.sh`, `dispatch-hygiene.sh`, and other hooks/scripts-only additions land without a bump).

### Changed
- **`hooks/scripts/version-stamp-check.sh`** (new): the check script.
- **`tests/version-stamp-check.test.sh`** (new): fixture-repo suite covering touched/untouched, bump/no-bump, and unmeasurable-range cases, plus two mutation controls.
- **`tests/validate.sh`**: registers the new suite.
- **`docs/trust-model.md`**: new row 25 for the script (`self-reported`, matching `heavy-trigger.sh`/`reviewer-tier.sh`'s classification).
- **`tests/trust-model-bijection.test.js`**: `EXPECTED_SELF_REPORTED_COUNT` 9 → 10.
- **`.claude/hooks/scripts/version-stamp-check.sh`** (new), **`.claude/persona-config.json`**: regenerated mirror and `fileHashes` entry via `node bin/cli.js --update`.
- Known side effect (not fixed in this unit, per project convention — a different unit's file): landing this 18th `hooks/scripts/*.sh` file makes `scripts/rollout-preflight.sh`'s hardcoded A24 script-count baseline (17) stale; `tests/rollout-preflight.test.sh` FAILs until a separate, scoped commit bumps it to 18.

**0.31.74 — Leaner escalation-packet authoring instructions across all four hand-synced copies (reviewer-changes-examples-lean-1, reviewer-changes-examples-lean-2).** Tightens the `CHANGES.md` / `EXAMPLES.md` authoring instructions in the `## Fourth verdict: escalate-to-human` section for leaner packets — shorter sentences, no filler, no restating — while keeping every load-bearing rule verbatim: the `Comprehension material only — the .escalated marker is the authoritative record.` first line, authored-after-verdict / never-gates, 3-5 worked examples at 2-4 lines each, never graded / never a gate, the `examples:` skip line, the fixed four-section shape, and the packet lifecycle. Unit -1 (`1a20c7d`) edited `templates/persona-protocol.md`; unit -2 (`c6c2476`) propagated the same tightening to the three independently hand-authored copies — `agents/reviewer.md` (the copy the reviewer persona actually executes, since `PROTOCOL_SECTIONS_BY_PERSONA.reviewer` drops the Fourth-verdict section from its inlined protocol block) and the two condensed adapter ports — and rescoped unit -1's closing line "Address what needs review; say nothing else." to "Within each section, say only what the human needs to review the change; nothing else.", in all four copies, so it cannot be read as licence to drop `## Background` or `## What this change is for`. Neither unit's commit bumped the version; this entry and the bump close constitution §3 / version-stamp discipline for the pair, since `bin/cli.js --update` early-exits on a stamped persona mirror by version comparison alone and an already-adapted project would otherwise never receive the change.

### Changed
- **`templates/persona-protocol.md`**: `## Fourth verdict: escalate-to-human` — `CHANGES.md` / `EXAMPLES.md` bullets tightened (unit -1); closing line rescoped to per-section (unit -2).
- **`agents/reviewer.md`**: "`CHANGES.md` — the literate change summary you owe the human" and "`EXAMPLES.md` — the worked examples you write but never grade" bullets tightened to match the template; closing line rescoped.
- **`adapters/codex/agents-md-fragment.md`**, **`adapters/cursor/rules/persona-protocol.mdc`**: the condensed `CHANGES.md` / `EXAMPLES.md` paragraphs tightened proportionally; closing line rescoped.
- **`.claude-plugin/plugin.json`**, **`package.json`**: version bump 0.31.73 → 0.31.74.
- **`.claude/agents/reviewer.md`**, **`.claude/persona-protocol.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update --force-render`; every other `.claude/agents/*.md` mirror (nine files) and `.claude/persona-protocol-slim.md`, `.claude/protocol-digest.md` also picked up the version stamp from the same regeneration.

**0.31.73 — Address three non-blocking advisory notes left on the rulings-ledger unit (orch-rulings-followup-1).** Adds `.claude/orchestrator-rulings.log` to `.gitignore`, matching the existing pattern for sibling runtime logs (`wip-audit.log`, `review-audit.log`, `dispatch-audit.log`, `microworld-audit.log`). Corrects the 0.31.72 CHANGELOG entry's regeneration bullet, which had named only `.claude/agents/orchestrator.md` and `.claude/persona-config.json`, to also disclose the nine other persona mirrors and three protocol/digest files that picked up the version-stamp ripple. Rewords the background-dispatch polling paragraph in `agents/orchestrator.md` so the ~5-10 minute polling gap is framed as a minimum-gap heuristic to apply across natural turn boundaries rather than implying a literal blocking wait, which the orchestrator has no primitive for; the bounded-gap and multi-dispatch-reconcile substance is unchanged.

### Changed
- **`.gitignore`**: new entry for `.claude/orchestrator-rulings.log`.
- **`CHANGELOG.md`**: corrected the 0.31.72 entry's regeneration bullet to name the full set of regenerated files.
- **`agents/orchestrator.md`**: reworded the bounded-wait polling sentence in "Managing a long-running background dispatch" to name an actionable mechanism (natural turn boundaries) instead of an implied timed wait.
- **`.claude-plugin/plugin.json`**, **`package.json`**: version bump 0.31.72 → 0.31.73.
- **`.claude/agents/orchestrator.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update --force-render`; every other `.claude/agents/*.md` mirror (nine files) and `.claude/persona-protocol.md`, `.claude/persona-protocol-slim.md`, `.claude/protocol-digest.md` also picked up the version stamp from the same regeneration.

**0.31.72 — Orchestrator gets a rulings ledger and a bounded-wait heuristic for polling background dispatches (orch-rulings-wait-heuristic).** Adds a "Rulings ledger" section to `agents/orchestrator.md`: a lightweight, append-only `.claude/orchestrator-rulings.log` line (`RULING <UTC ISO-8601 timestamp> unit=<task-id|n/a> decision=<short summary>`) for judgment calls the orchestrator was already authorized to make on its own (e.g. a downgrade-only reviewer-tier override) — explicitly not a gate, not hook-checked, and never a substitute for the `AskUserQuestion` / `ESCALATE-TO-HUMAN` paths documented elsewhere in the file. Also amends the existing background-dispatch polling guidance to wait in bounded ~5-10 minute stretches between `TaskOutput` polls (rather than polling rapidly or waiting indefinitely) and to reconcile against other outstanding live dispatches at the same time, rather than checking each one in isolation. Resubmitted after a FAIL that flagged a missing version bump/CHANGELOG entry and an internal contradiction in the ledger's own example list (two of its three examples named human-gated decisions); the version-bump/CHANGELOG gap is closed by this entry, and the example list is corrected to keep only the one example that is genuinely self-authorized.

### Changed
- **`agents/orchestrator.md`**: new "Rulings ledger" section; background-dispatch polling guidance amended with a bounded-wait/reconcile heuristic.
- **`.claude-plugin/plugin.json`**, **`package.json`**: version bump 0.31.71 → 0.31.72.
- **`.claude/agents/orchestrator.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update --force-render`; every other `.claude/agents/*.md` mirror (nine files) and `.claude/persona-protocol.md`, `.claude/persona-protocol-slim.md`, `.claude/protocol-digest.md` also picked up the version stamp from the same regeneration.

**0.31.71 — Vendor `grill-with-docs`, pivot `spec-master` onto it, and record the reversal of ADR-0012 (gwd-1..4, docs/plans/2026-09-11-grill-with-docs-vendoring-and-spec-master-pivot.md, all four units).** Vendors upstream `grill-with-docs` as a `fm-noflag` declared deviation, converts the existing `grill-me` from byte-verbatim `fm` to `fm-noflag`, and extends `scripts/resync-vendored-skills.sh` to drift-check both (gwd-1). Records ADR-0031, the reversal of ADR-0012's "`grill-me` is the control" decision, with a reciprocal `Amended by ADR-0031` marker on ADR-0012 (gwd-2). Brings the runbook, README credit line, and `CONTEXT.md` glossary (new `grill-with-docs` entry) back into agreement with `gwd-1`'s vendoring (gwd-3). Pivots `agents/spec-master.md`'s interrogation step from bare `grill-me`/`grilling` to `grill-with-docs`, adds `domain-modeling` and `grill-with-docs` to its `skills:` preload line, mirrors the same rewording into `skills/ubiquitous-language/SKILL.md`, and adds a `tests/validate.sh` check that every `agents/*.md` `skills:` frontmatter token resolves to a `skills/<x>/SKILL.md` — the merge-gate guard that makes the wiring claim checkable, closing the gap `grilling-fix-1.fail` identified (gwd-4). This entry, the version bump, and a full `node bin/cli.js --update --force-render` mirror regeneration land in gwd-4's own commit, per constitution §3, so the spec-master mirror's `fileHashes` entry and the re-stamp of every `.claude/agents/*.md` to v0.31.71 in that commit is expected `--force-render` output, not a hand-edit.

### Changed
- **`skills/grill-with-docs/`** (new, vendored `fm-noflag`), **`skills/grill-me/SKILL.md`** (converted `fm` → `fm-noflag`), **`scripts/resync-vendored-skills.sh`**: drift-checks both skills.
- **`docs/adr/0031-grill-with-docs-model-invocable.md`** (new), **`docs/adr/0012-*.md`**: reciprocal `Amended by ADR-0031` marker.
- **`docs/runbooks/`**, **`README.md`**, **`CONTEXT.md`**: vendoring table/counts, credit line, and new `grill-with-docs` glossary entry brought back into agreement with `gwd-1`.
- **`agents/spec-master.md`**: `skills:` line gains `antislop:grill-with-docs` and `antislop:domain-modeling`; `grill-me`/`grilling` prose repointed to `grill-with-docs` throughout (6 occurrences); new sentence on `domain-modeling`'s glossary/ADR-drafting duty.
- **`skills/ubiquitous-language/SKILL.md`**: matching `grill-me` → `grill-with-docs` rewording.
- **`tests/validate.sh`**: new check — every `agents/*.md` `skills:` frontmatter `antislop:<x>` token must resolve to `skills/<x>/SKILL.md`.
- **`.claude-plugin/plugin.json`**, **`package.json`**: version bump 0.31.70 → 0.31.71.
- **`.claude/agents/*.md`** (all ten), **`.claude/persona-protocol.md`**, **`.claude/persona-protocol-slim.md`**, **`.claude/protocol-digest.md`**, **`.claude/persona-config.json`**: regenerated via `node bin/cli.js --update --force-render`.
...
### Changed
- **`hooks/scripts/stop-gate.sh`** (step 0.5 comment): scoped marker check described; no longer claims directory-wide glob.
- **`hooks/scripts/lib/stop-gate-core.sh`**, **`adapters/codex/hooks/scripts/lib/stop-gate-core.sh`**, **`adapters/cursor/hooks/scripts/lib/stop-gate-core.sh`**: review-join state consulted before marker check; scoped unit set gates marker relevance; `marker-out-of-scope` logged per skipped marker.
- **`hooks/scripts/reviewer-route-gate.sh`**, **adapter ports**: review-join stamps bound to scoped-set building (no logic change, mirrored parity maintained).
- **`tests/marker-write.test.sh`**, **`hooks/scripts/marker-write.sh`**, **`.claude/hooks/scripts/marker-write.sh`**: `CLAUDE_PROJECT_DIR` pinned on all helper invocations; `.dot` override-friendly per state-access.sh pattern.
- **`tests/validate.sh`**: standing guard registered asserting `.claude/reviewed/` entry list unchanged across suite run.
- **`CONTEXT.md`**: new glossary entries for `scoped unit set`, `marker relevance scoping`, and `marker-out-of-scope` audit token.
- **`templates/persona-protocol.md`**: one-unit-at-a-time invariant now documented as complemented (not solely depended upon) by scoped-relevance mechanism; reference to ADR-0028 added.
- **`.claude/persona-config.json`**, **`.claude/hooks/scripts/`** mirrors: regenerated via `node bin/cli.js --update --force-render`.
...
### Changed
- **`hooks/scripts/harness-integrity-gate.sh`** (new): the gate script.
- **`hooks/hooks.json`**: registers the new gate on both the `Write|Edit` and `Bash` `PreToolUse` matchers.
- **`tests/harness-integrity-gate.test.sh`** (new): Set A/Set B coverage, Bash GUARD carve-outs, hot-path line-number ordering assertion, and two mutation controls (branch-disjointness, Set-B-on-Bash asymmetry regression).
- **`tests/protected-paths-coverage.test.js`**: adds a reasoned exemption entry for the new, self-protecting gate script.
- **`.claude/hooks/scripts/harness-integrity-gate.sh`** (new), **`.claude/persona-config.json`**: regenerated mirror and `fileHashes` entry via `node bin/cli.js --update`.

**Append-only audit-log seals: `audit_append`/`audit_seal_verify`/`audit_rotate` (gh415, Step 3 of docs/plans/2026-08-25-harness-trust-gaps.md, complete — all 23 append sites converted).** New `hooks/scripts/lib/audit-log.sh` makes erasing an audit log detectable: `audit_append` appends then rewrites a `<log>.seal` sidecar (`lines=<N> sha256=<hash>`); `audit_seal_verify` reports `ok|truncated|missing-seal|absent|unverifiable`; `audit_rotate` archives a log to `<dot>/audit-archive/` and reseals a fresh one. New `bin/harness-integrity.sh` (always exit 0) reports `harness-integrity=<ok|drift|tampered|unverifiable> config=<ok|drift|missing> logs=<ok|LIST>`, with a `--rotate` mode. `bin/human-review-cleanup.sh`'s pre-existing log-rotation sweep now reseals via the same library after preserving its tail line (for `stop-gate.sh`'s defer-dedup), so a legitimate `--apply` rotation never reads as tampering to `audit_seal_verify`. Every gate and shared library that appended to an audit log (`dispatch-hygiene.sh`, `human-decision-gate.sh`, `reviewed-path-gate.sh`, `task-gate.sh`, `lib/stop-gate-core.sh`, `lib/microworld-rerun-core.sh`, `lib/reviewer-route-gate-core.sh`, `lib/agent-identity.sh`, `lib/microworld-queue.sh`) now calls `audit_append` instead of a raw `>>` redirection; the written log-line format is unchanged. `hooks/scripts/lib/audit-log.sh` is documented as detecting accidental truncation/deletion, not an adversary who truncates and reseals. `.gitignore` gains `.claude/*.log.seal` and `.claude/audit-archive/`. `.claude/persona-config.json`'s `protectedPaths` is unchanged in scope by Step 3 (the 5 existing entries for the converted `lib/*.sh` files are retained per spec 2 Unit E's coverage policy — being a critical/shared hook library, not the append-append style, is what earns protectedPaths coverage); one new entry is added for the new `hooks/scripts/lib/audit-log.sh` file itself.

### Changed
- **`hooks/scripts/lib/audit-log.sh`** (new), **`bin/harness-integrity.sh`** (new): the seal library and its read-only reporter/rotator CLI.
- **`bin/human-review-cleanup.sh`**: sources `lib/audit-log.sh`; `rotate_log()` reseals the fresh (tail-preserved) log after archiving.
- **`bin/cli.js`**: `SHARED_HOOK_LIB_FILES` gains `'audit-log.sh'`.
- **`hooks/scripts/dispatch-hygiene.sh`**, **`hooks/scripts/human-decision-gate.sh`**, **`hooks/scripts/reviewed-path-gate.sh`**, **`hooks/scripts/task-gate.sh`**, **`hooks/scripts/lib/stop-gate-core.sh`**, **`hooks/scripts/lib/microworld-rerun-core.sh`**, **`hooks/scripts/lib/reviewer-route-gate-core.sh`**, **`hooks/scripts/lib/agent-identity.sh`**, **`hooks/scripts/lib/microworld-queue.sh`**: raw `>>` appends replaced with `audit_append`.
- **`hooks/scripts/stop-gate.sh`**, **`hooks/scripts/microworld-rerun.sh`**, **`hooks/scripts/reviewer-route-gate.sh`**: now source `lib/audit-log.sh` before their extracted core/queue logic.
- **`tests/audit-seal.test.sh`** (new), **`tests/fixtures/seed-truncated-log.sh`** (new): C3.3/C3.4 coverage.
- **`tests/stop-gate-deferred-microworld.test.sh`**, **`tests/stop-gate-microworld-skip.test.sh`**: isolated-fixture mutation harnesses now also copy `lib/audit-log.sh` alongside `lib/agent-identity.sh`, matching `stop-gate.sh`'s new dependency.
- **`tests/human-review-cleanup.test.sh`**: archive-detection globs now exclude `*.seal` so the new sidecar isn't mistaken for a rotated archive.
- **`.gitignore`**: `.claude/*.log.seal`, `.claude/audit-archive/`.
- **`.claude/persona-config.json`**: `protectedPaths` unchanged for the 5 existing entries (`hooks/scripts/lib/{agent-identity,microworld-rerun-core,microworld-queue,stop-gate-core,reviewer-route-gate-core}.sh` remain covered); one new entry added for `hooks/scripts/lib/audit-log.sh`.
- **`adapters/codex/hooks/scripts/lib/audit-log.sh`**, **`adapters/cursor/hooks/scripts/lib/audit-log.sh`** (new): adapter mirrors, plus regenerated `.claude/hooks/scripts/*` mirrors and `.claude/persona-config.json`'s `fileHashes` via `node bin/cli.js --update`.
...
**Fix `_memo_setup`'s suite memo defeating mutation-proof microworld bundles (memo-key-1, 2026-08-26).** The suite-level memoizing `bash` wrapper in `hooks/scripts/lib/microworld-queue.sh` keyed its cache on the test file path alone, so a bundle invoking the same suite twice under a differing environment (e.g. `GATE_UNDER_TEST=...`, the pattern `microworlds/rpg-canon-2` and `microworlds/hdg-anchor-1` both use) was served the first call's cached exit code instead of re-executing — a false-negative mutation proof, already flagged as an unresolved carried-forward finding in `spec2-unitA.pass` note N6. The cache key now includes a `cksum` digest over the full argv and `env | sort`, plus a per-shell `<key>.$$.seen` guard so a same-environment in-tree mutation is never served a stale hit either; an existing `.rc` file is never overwritten. `_drain_loop` also flushes the memo's `results/` directory at the top of every outer pass, so a second drain pass (triggered by a fresh edit) never reuses a first-pass result. AC-A4 suite-level dedup (one execution shared across bundles invoking the same unmutated suite) is unregressed. No version bump (P3 not triggered — no version-stamped files touched).

### Changed
- **`hooks/scripts/lib/microworld-queue.sh`**: `_memo_setup`'s cache key composition and `.seen` guard; `_drain_loop`'s per-pass memo flush.
- **`tests/microworld-rerun.test.sh`**: two new regression cases, `(k)` (mutation-proof: differing env prefix yields a different exit code even with the cache pre-populated) and `(l)` (per-pass flush: a bundle re-enqueued mid-run is re-executed in drain pass 2, not served pass 1's result). Both mutation-proved in both directions.
- **`.claude/hooks/scripts/lib/microworld-queue.sh`** / **`adapters/codex/hooks/scripts/lib/microworld-queue.sh`** / **`adapters/cursor/hooks/scripts/lib/microworld-queue.sh`** / **`.claude/persona-config.json`**'s `fileHashes`: regenerated via `node bin/cli.js --update --force-render` (G1/G2).

**Fix `rollout-preflight.sh`'s staleness gate reporting a false, permanent `unmet-on-staleness` for every spec without a `--reverify` implementation (rollout-reverify-gap, 2026-08-26).** `wave_staleness_check` compared each owning spec's plan-doc mtime against a `--reverify` stamp file that only `reverify_spec6` can ever write, so specs 1-4 (owning W1, W4, W5, W6, W7, W10) reported `unmet-on-staleness` unconditionally and indistinguishably from a real staleness finding. A new `REVERIFY_IMPLEMENTED` table names which specs have a real `--reverify` implementation; `wave_staleness_check` now prints a distinct `no-reverify-support` line for specs without one instead of the misleading `unmet-on-staleness`, and does not count it toward the stale return value. `reverify_spec` dispatches off the same table instead of a separate hardcoded case, so the two never drift. Spec 6's staleness reporting (W0, W8, W9) is unchanged (confirmed byte-identical before/after). No version bump (P3 not triggered — no version-stamped files touched).

### Changed
- **`scripts/rollout-preflight.sh`**: `REVERIFY_IMPLEMENTED` table (new), `reverify_supported()` helper (new), `wave_staleness_check`'s no-reverify-support branch, `reverify_spec`'s dispatch now reads `REVERIFY_IMPLEMENTED` instead of a separate case list.
- **`tests/rollout-preflight.test.sh`**: two new mutation-proved assertions -- W1 (spec 3, no support) reports `no-reverify-support` and never `unmet-on-staleness`; W9 (spec 6, has support) keeps its `unmet-on-staleness` wording unchanged.
...
### Changed
- **`templates/persona-protocol.md`** / **`agents/reviewer.md`**: same soft-cap (400 → 120) and terseness/brevity wording the mirrors already carried, now in the actual generator sources.
- **`.claude/agents/*.md`** / **`.claude/persona-protocol*.md`** / **`.claude/protocol-digest.md`** / **`.claude/persona-config.json`**'s `fileHashes`: regenerated via `node bin/cli.js --update` (G1/G2) — content was already byte-identical, so this run only healed stale hashes and refreshed version stamps.
- **`adapters/codex/agents-md-fragment.md`** / **`adapters/cursor/rules/persona-protocol.mdc`**: same cap and brevity wording hand-ported into both house styles for consistency (not test-gated, but the same hand-duplicated instruction text).
- **`CONTEXT.md`**: **literate change summary** glossary entry's stale "soft cap 400 lines" corrected to 120.
- **`.claude-plugin/plugin.json`** / **`package.json`**: version bump 0.31.62 → 0.31.63 (constitution P3).

## [0.31.62] - 2026-08-20

**Replace the escalation packet's comprehension quiz with worked examples in the three normative sources (examples-1, Step 1 of the 2026-08-20 quiz-to-worked-examples plan).** `QUIZ.md` + `QUIZ-ANSWERS.md` — 3 to 5 self-administered questions with a separate answer key — are retired in favor of a single `EXAMPLES.md`: 3 to 5 worked examples, each a behavioural before/after grounded in `CHANGES.md` and the bundle, about consequence rather than recall. Examples are written whenever the change has an observable behavioural consequence and skipped for pure docs/formatting/comment/rename changes; a skip is now auditable — the `.escalated` marker body gains an `examples: <count>` or `examples: none — <reason>` line, which `PACKET.md` inherits automatically as a byte-identical copy. The reviewer's non-adjudication constraint (never grade, never gate) is retained and stated explicitly, now structurally trivial with no answer key but written down so a later editor cannot reintroduce grading. The approve-route attestation's `quiz:` token is renamed to `examples:`, with the token vocabulary changing from `passed-self-check | skipped | none-offered` to `reviewed | skipped | none-offered`; every existing rule carries over verbatim under the new name (first-class legitimate skip, approve-route only, absent-line fallback, marker-format safety). Landed in four units under one version, since only the first bumped the plugin version: examples-1 (protocol sources + mirrors, this paragraph), examples-2 (hand-maintained adapter ports + parity probes), examples-3 (dashboard composer/server/reader/client + tests), and examples-4 (this glossary/README/changelog documentation pass). No migration was needed for historical `.pass` markers or in-flight packets: `.claude/human-review/` was empty throughout (zero escalations mid-flight), and `task-gate.sh`'s `marker_valid()` reads a marker's required first line only, so pre-existing markers carrying an old appended `quiz:` line remain valid untouched.

### Changed
- **`templates/persona-protocol.md`**: "Fourth verdict: escalate-to-human" section — packet-creation bullet writes `EXAMPLES.md` in place of the quiz pair, with the when-needed rule and the `.escalated` marker's auditable-skip line; the approve-route attestation and its token-rules subsection renamed `quiz:` → `examples:` (`reviewed | skipped | none-offered`); all deletion/lifecycle bullets updated to name `EXAMPLES.md`.
- **`agents/reviewer.md`**: same substance ported into the reviewer's first-person voice — packet-creation, the `EXAMPLES.md` bullet, the approve-route transcription rules, and the packet-deletion list.
- **`agents/orchestrator.md`**: surfaces `EXAMPLES.md` in place of the quiz; the never-answer-the-quiz rule becomes never author/extend examples on the human's behalf; the `DECISION` template's approve-route third line becomes `examples: <token>`.
- **`.claude-plugin/plugin.json`** / **`package.json`**: version bump 0.31.61 → 0.31.62 (constitution P3).
- **`.claude/agents/*.md`** / **`.claude/persona-protocol*.md`** / **`.claude/protocol-digest.md`** / **`.claude/persona-config.json`**'s `fileHashes`: regenerated via `node bin/cli.js --update --force-render` (content) then `node bin/cli.js --update` (version-stamp refresh across all managed mirrors) (G1/G2).
- **`adapters/codex/agents-md-fragment.md`** / **`adapters/cursor/rules/persona-protocol.mdc`** (examples-2): hand-ported the same protocol change into both house styles.
- **`tests/adapter-protocol-parity.test.js`** (examples-2): `ESCALATION_PROBES` swapped the quiz literals for `EXAMPLES.md` / `examples: reviewed` / `examples: skipped` / `examples: none-offered`, with a mutation proof confirming the probe array actually gates on the new concept.
- **`bin/microworld-dashboard/decision-block.js`** / **`server.js`** / **`decisions.js`** / **`index.html`** (examples-3): `QUIZ_TOKENS` → `EXAMPLES_TOKENS` (`reviewed | skipped | none-offered`), the `quiz` context/form field renamed `examples` throughout, `EXAMPLES.md` read in place of `QUIZ.md`, and the answer-key reveal button, its container, and its lazy `/api/source` fetch deleted (the endpoint itself and its other two consumers are untouched).
- **`tests/dashboard-decision-block.test.js`** / **`dashboard-decision-run.test.js`** / **`dashboard-decisions.test.js`** / **`dashboard-decisions-client.test.js`** (examples-3): retargeted to the new vocabulary, one-for-one with each existing check's intent.
- **`CONTEXT.md`** (examples-4): **comprehension quiz** entry marked `[Retired in 0.31.62; see worked example below.]`, historical body kept; new **worked example** entry added as a subtype of **comprehension material**; **Escalation packet** entry names `EXAMPLES.md`; **Document pane** entry's enumeration drops the two retired quiz panes and gains `EXAMPLES.md`, six document panes renumbered to five.
- **`README.md`** (examples-4): "What's in the packet" and the following paragraph rewritten for `EXAMPLES.md` and the `examples:` token vocabulary, keeping the reader-facing reassurance that the material is theirs and a skip is legitimate.

## [0.31.61] - 2026-08-16

**Turn off `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` by default and document the confirmed roster/dispatch mitigations from the agent-teams research report.** A prior investigation (confirmed against Anthropic's own docs at https://code.claude.com/docs/en/agent-teams) established two facts this project had not yet acted on: **Fact A** — a teammate roster row that disappears after sitting idle "has been hidden, not stopped" (rows hide ~30s after the whole panel goes idle, more than 3 idle teammates collapse into a single `N idle agents` row, and a hidden row reappears on the teammate's next turn), so an absent row is never by itself proof a teammate terminated; **Fact B** — while `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` is enabled, *any* named subagent dispatch launches as a teammate whose completion surfaces only as a content-free idle notification unless it explicitly `SendMessage`s back, and per the docs verbatim, "An orchestration flow that waits on subagent results can stall." Since this project's own default dispatch style is unnamed specifically to avoid that stall, and the flag was nonetheless shipping `"1"` in both `.claude/settings.json` and `templates/settings-fragment.json`, every fresh install and this repo's own dev session carried the exact failure mode the "prefer unnamed dispatch" rule exists to avoid. The flag now defaults to `"0"`; agent-teams mode remains available as a deliberate per-task opt-in via `/antislop:start-feature-team`. Re-fetching the docs also settled an open question about the re-enable mechanism: Claude Code "reapplies settings-file `env` values to the running session when you save, and rereads the variable each time Claude spawns a subagent" — so flipping the value back to `"1"` in `.claude/settings.json` and saving takes effect on a live session without a restart; this is now documented as the verified mechanism rather than assumed.

### Changed
- **`.claude/settings.json`**: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` changed `"1"` → `"0"`.
- **`templates/settings-fragment.json`**: same value change for new installs; `_comment` rewritten to state the off-by-default rationale (Fact B, cited with the doc URL and quote), point at `/antislop:start-feature-team` as the opt-in, and note the verified live-reapply re-enable mechanism, while keeping the existing default-teammate-model caveat intact.
- **`commands/start-feature-team.md`**: new precondition note near the top stating the flag now defaults off and documenting the verified toggle-without-restart mechanism (edit `.claude/settings.json`, save, no restart needed).
- **`agents/orchestrator.md`**: "Default dispatch naming: unnamed vs. named" section gains a paragraph citing Fact B as the now-confirmed mechanism behind the unnamed-dispatch default and cross-referencing the flag's new off-by-default shipped setting; "Managing a long-running background dispatch" section gains a new "A missing roster row is not proof a teammate is dead" subsection documenting Fact A, the `SendMessage`-first diagnostic before assuming a teammate is dead, and the escalation condition for filing a bug against `anthropics/claude-code` (a `SendMessage` failure while `~/.claude/teams/session-*/config.json` still lists the member as active, with that config snapshot attached).
- **`.claude-plugin/plugin.json`** / **`package.json`**: version bump 0.31.60 → 0.31.61 (constitution P3).
- **`.claude/agents/*.md`**: regenerated via `node bin/cli.js --update` (G2).
- **`agents/spec-master.md`**: replaced the fixed "at most 5 questions total" interrogation cap with a per-round batch limit of ≤4 questions (`AskUserQuestion`'s own per-call schema limit) plus an explicit convergence-based stopping rule — keep returning Open Questions rounds until every taxonomy category is Clear or explicitly deferred as a named assumption, rather than stopping once a question budget is spent.
- **`agents/milestone-auditor.md`**: fixed a stale `grill-me`-as-invocation-target reference (`grill-me` is a disabled, non-invocable pointer; the actually invocable skill is `grilling`), using the same disambiguation parenthetical already used in `agents/spec-master.md`.

## [0.31.60] - 2026-08-16

**Reconcile the shared 2-FAIL-cap paragraph and fix a stale publish-threshold branch (adhoc-2026-08-16-cap-paragraph-reconcile, Step 1 of the 2026-08-16 ceremony-reduction-followups plan).** `templates/persona-protocol.md`'s "Cap at 2 FAILs per unit" paragraph still described the automatic `spec-master` debug-spec spawn that 0.31.59 removed in favor of a human `AskUserQuestion` gate; it now states that the human is asked how to proceed and points at `agents/orchestrator.md`'s own "At the 2-FAIL cap" section as the single source of truth, rather than restating (and drifting from) it. Separately, `agents/spec-master.md`'s publish-optional branch still read `<3 units`, leaving 3-, 4-, and 5-unit specs matching neither it nor the `≥6 units` publish branch; it is now `≤5 units`, making the two branches complementary and exhaustive.

### Changed
- **`templates/persona-protocol.md`**: rewrote the "Cap at 2 FAILs per unit" paragraph to defer to `agents/orchestrator.md`'s "At the 2-FAIL cap" section instead of restating the removed automatic-spawn behavior.
- **`agents/spec-master.md`**: fixed the stale `<3 units` publish-optional branch to `≤5 units`.
- **`.claude-plugin/plugin.json`** / **`package.json`**: version bump 0.31.59 → 0.31.60 (constitution P3).
- **`.claude/agents/*.md`** / **`.claude/persona-protocol*.md`** / **`.claude/persona-config.json`**'s `fileHashes`: regenerated via `node bin/cli.js --update --force-render` (G2). The Codex/Cursor/prototype ports already carried the correct wording and were not touched.
...
### Changed
- **`templates/persona-protocol.md`**: `## Fourth verdict: escalate-to-human` gains the `QUIZ.md` / `QUIZ-ANSWERS.md` creation bullet (question shape, separate answer key, the never-graded/never-a-gate authority rule, after-the-verdict ordering); the packet-lifecycle bullet names both files. "Resolving an escalation" gains the approve-row `quiz: <token>` on the attestation line, the three token values, the approve-route decision-file template with its `quiz:` body line, the first-class-skip rule (including the absent-line fallback), the approve-route-only rule, and the marker-format-safety note; the three-route deletion rule now names both files.
- **`adapters/codex/agents-md-fragment.md`** / **`adapters/cursor/rules/persona-protocol.mdc`**: hand-adapted condensed ports of the same creation, token, and deletion rules.
- **`agents/reviewer.md`**: authorship bullet (question shape, separate key, never-graded), the approve-route transcription rule (required token, verbatim from the decision body, absent-line fallback, no token on reject/direct), and the three-route deletion rule now naming both files.
- **`agents/orchestrator.md`**: new surfacing step in the `ESCALATE-TO-HUMAN` list — surface `QUIZ.md` **(if present)** and **must not answer the quiz** on the human's behalf, the exact analogue of the existing never-run-`run.sh`-and-pre-digest rule; the decision-template step gains the approve-route `quiz: <token>` line and the relay-verbatim / never-nag-a-skip rule; subsequent steps renumbered.
- **`README.md`**: "What's in the packet" names both files, plus a reader-facing paragraph on the three token values and the quiz being theirs rather than a test to pass.
- **`CONTEXT.md`**: glossary entry **comprehension quiz** (recorded, never graded, never gating, approve route only, the three tokens, marker-format safety, `_Avoid_` synonyms); the **Escalation packet** entry now names both files.
- **`tests/adapter-protocol-parity.test.js`**: the escalate-to-human parity row now takes an **array** of probes rather than a single header string, asserting both ports actually carry `QUIZ.md`, `QUIZ-ANSWERS.md`, all three `quiz:` tokens, `never graded by the reviewer`, and gh299's `Comprehension material only` sentinel. Closes the gap gh299's reviewer flagged: a header-only probe passed on a port that had silently dropped the packet's human-facing artifacts.
- Regenerated the six full-protocol `.claude/agents/*.md` copies and `fileHashes` via `node bin/cli.js --update` (G2).

## [0.31.54] - 2026-08-15

**Add `CHANGES.md`, a literate change summary, to the escalation packet (gh299, Step 10 of the 2026-08-09 human-review convergence follow-ups).** At escalation the human previously received a run command, a one-line inputs/expected description, the reviewer's would-be verdict and notes, and the bundle's `README.md` describing *the piece* — nothing describing *the change*, so forming an opinion meant reading a raw, alphabetically ordered diff. The reviewer now writes `CHANGES.md` into `.claude/human-review/<task-id>/` in the same action that writes the `.escalated` marker and `PACKET.md`: no new tool, no rendering pipeline, no HTML, just the reading of the diff that already produced the would-be verdict, written down. Fixed four-section shape so it cannot decay into a restated diff — `## Background`, `## What this change is for`, `## Walkthrough` (conceptual order, one subsection per idea, explicitly **not one subsection per file** and not alphabetical), `## What to look at first`. It quotes the diff rather than reproducing it, soft cap 400 lines, and its first line is byte-exact `Comprehension material only — the .escalated marker is the authoritative record.` — the same authority rule `PACKET.md` carries, applied to a second derived artifact. Authored *after* the would-be verdict is reached; it never gates or influences that verdict. A unit with `microworld: none` still gets one, and there it carries the entire human-facing payload. Deleted with the rest of the packet in all three of the DECISION channel's terminal routes, in the same reviewer action. Extends the existing `## Fourth verdict: escalate-to-human` section and its "Resolving an escalation" subsection — **no new top-level protocol section**, so the per-persona section matrix and the adapter parity map are untouched.

### Changed
- **`templates/persona-protocol.md`**: `## Fourth verdict: escalate-to-human` gains the `CHANGES.md` creation bullet (four-section shape, authority first line, quote-don't-reproduce, 400-line soft cap); the no-bundle bullet and the packet-lifecycle bullet now name it; the "Resolving an escalation" subsection states its deletion alongside `PACKET.md` and the bundle copy in all three routes.
- **`adapters/codex/agents-md-fragment.md`** / **`adapters/cursor/rules/persona-protocol.mdc`**: hand-adapted condensed ports of the same creation and deletion rules.
- **`agents/reviewer.md`**: authorship bullet (shape, authority line, after-the-verdict ordering, no-bundle case); the three-route deletion rule now names `CHANGES.md`.
- **`agents/orchestrator.md`**: new surfacing step in the `ESCALATE-TO-HUMAN` list — point the human at `CHANGES.md` **(if present)** and tell them to read it *before* the diff; subsequent steps renumbered.
...
**Document `humanReviewMode`'s on-by-default posture, the microworld bundle format, and three known limitations in `README.md` (gh137, Step 8a of the microworlds/ubiquitous-language/human-review spec).** README-only documentation unit. States plainly, without softening: `humanReviewMode` defaults to `critical`, not `off` — a real, intentional behaviour change for every already-adapted project on its next plugin update, chosen deliberately to counteract hyperscaling slop (this section already existed from Step 6's unit; unchanged here). Adds a new "Microworld bundles" section documenting the `microworlds/<unit-slug>/` layout (`manifest.json`, `run.sh`, `inputs/`, `expected/`, `README.md`) and its gitignored-scratch storage, including that a bundle is expected to be absent in a fresh clone and is never part of a reviewed diff. Extends the `antislop:ubiquitous-language` skill's Credits description (added by an earlier unit) rather than duplicating the skills-list line. Adds a "Reviewer and escalation limitations" subsection under "Known limitations" recording three items as documentation, not fixes: (a) R5 — a stale `.escalated` marker survives `skip:`, shared with the existing `.blocked` marker's pre-existing shape; (b) `package.json`'s `files` array shipping only 2 of the `skills/` directory's 17 skills is a recorded decision (skills reach users via git/marketplace, not npm), not a bug; (c) R10 — escalation packets at `.claude/human-review/<task-id>/` are untracked and do not survive `git clean -fdx`, a fresh clone, or a discarded worktree, an accepted consequence of keeping review scratch out of project history.

### Changed
- **`README.md`**: added "Microworld bundles" section (format + gitignored-scratch storage); extended the `antislop:ubiquitous-language` Credits bullet; added a "Reviewer and escalation limitations" subsection under "Known limitations" documenting R5, the `package.json` `files`-array gap, and R10.
- **`.claude-plugin/plugin.json`** / **`package.json`**: version bump 0.31.54 → 0.31.55 (constitution P3).
- **`.claude/persona-config.json`**: `pluginVersion` bump to match.
...
### Changed
- **`agents/reviewer.md`, `templates/persona-protocol.md`** (the two locations the spec named that actually carried the false claim): replaced the "read-only ones included" sentence with wording naming `command_is_provably_benign()` and its allowlist/no-tolerance mechanism, preserving the surviving conclusion — an escalation packet must not be sited under `.claude/reviewed/` because a `run.sh` there is never on the allowlist.
- **`adapters/codex/agents-md-fragment.md`, `adapters/cursor/rules/persona-protocol.mdc`**: the spec's dispatch packet listed `agents/orchestrator.md` as the third location, but that file was verified (full read, all 547 lines) to carry no such claim at all — it never discusses why the escalation packet avoids `.claude/reviewed/`, only that the reviewer already placed it under `.claude/human-review/`. A repo-wide grep for the false sentence found two hand-authored adapter sources carrying it instead: this Codex adapter fragment (not auto-regenerated by `--update`, since no `.codex/` mirror exists in this repo) and the Cursor adapter's `persona-protocol.mdc` (likewise hand-authored, not auto-regenerated — no `.cursor/` mirror exists in this repo either). Both were corrected the same way, each matching its own file's plain-ASCII-hyphen style. The false claim existed in four locations total across the corpus, not three; all four are now corrected.
- **`.claude/agents/reviewer.md`, `.claude/persona-protocol.md`, `.claude/persona-config.json` (`fileHashes`)**: mirrors regenerated via `node bin/cli.js --update`.

### Notes
- `hooks/scripts/reviewed-path-gate.sh` and `hooks/scripts/lib/benign-command.sh` are untouched — this step is prose-only, documenting the gate's real behavior, never changing it.
