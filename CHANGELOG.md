...
**Append-only audit-log seals: `audit_append`/`audit_seal_verify`/`audit_rotate` (gh415, Step 3 of docs/plans/2026-08-25-harness-trust-gaps.md, partial — see Known gap).** New `hooks/scripts/lib/audit-log.sh` makes erasing an audit log detectable: `audit_append` appends then rewrites a `<log>.seal` sidecar (`lines=<N> sha256=<hash>`); `audit_seal_verify` reports `ok|truncated|missing-seal|absent|unverifiable`; `audit_rotate` archives a log to `<dot>/audit-archive/` and reseals a fresh one. New `bin/harness-integrity.sh` (always exit 0) reports `harness-integrity=<ok|drift|tampered|unverifiable> config=<ok|drift|missing> logs=<ok|LIST>`, with a `--rotate` mode. `bin/human-review-cleanup.sh`'s pre-existing log-rotation sweep now reseals via the same library after preserving its tail line (for `stop-gate.sh`'s defer-dedup), so a legitimate `--apply` rotation never reads as tampering to `audit_seal_verify`. `dispatch-hygiene.sh`, `human-decision-gate.sh`, `reviewed-path-gate.sh`, and `task-gate.sh` now call `audit_append` instead of a raw `>>` redirection; the written log-line format is unchanged. `hooks/scripts/lib/audit-log.sh` is documented as detecting accidental truncation/deletion, not an adversary who truncates and reseals. `.gitignore` gains `.claude/*.log.seal` and `.claude/audit-archive/`.

**Known gap, not closed in this unit.** 5 of the spec's 23 append sites remain unconverted — `hooks/scripts/lib/stop-gate-core.sh` (13), `hooks/scripts/lib/microworld-rerun-core.sh`, `hooks/scripts/lib/reviewer-route-gate-core.sh`, `hooks/scripts/lib/agent-identity.sh`, `hooks/scripts/lib/microworld-queue.sh` — all five are in `.claude/persona-config.json`'s `protectedPaths` and not in this unit's protectedPaths-lift list, so C3.2's backstop sweep is still RED. `stop-gate.sh`, `microworld-rerun.sh` and `reviewer-route-gate.sh` (the entry points, unblocked) now source `lib/audit-log.sh` ahead of their cores/identity library so `audit_append` is in scope once those five sites convert. Blocked pending a human protectedPaths decision; see the gh415 WIP sentinel / ready-for-review report for the full account.
...
- **`hooks/scripts/dispatch-hygiene.sh`**, **`hooks/scripts/human-decision-gate.sh`**, **`hooks/scripts/reviewed-path-gate.sh`**, **`hooks/scripts/task-gate.sh`**: raw `>>` appends replaced with `audit_append`.
