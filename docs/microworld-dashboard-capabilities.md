...
| `GET /api/decisions` | four human-decision touchpoints | load-bearing | ADR 0018; inert while `humanReviewMode: "off"`; CHANGES.md gap closed at `0779929`, covered by Test (i) in `dashboard-decisions.test.js` and Test (g) in `dashboard-decisions-client.test.js` |
