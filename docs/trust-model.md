...
| 25 | A unit touching `agents/*.md` or `templates/` bumped `.claude-plugin/plugin.json`'s version in the same range (constitution P3 / version-stamp discipline) | `hooks/scripts/version-stamp-check.sh` | self-reported | Deliberately **not** hook-registered — a reviewer-invoked helper, matching `heavy-trigger.sh`/`reviewer-tier.sh` (unit version-stamp-guard-1). Deterministic given an explicit `<commit-range>` argument, sidestepping CI's shallow clone; nothing independently re-derives whether the reviewer actually ran it against the right range. |
