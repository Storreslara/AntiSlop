...
| 11 | The harness is armed (its trust gates have not been silently disarmed) | — | `hooks/scripts/harness-integrity-gate.sh` | Configless, hardcoded write-deny over the harness's own control surface. Complemented by six gates' `harness_armed()` adoption, which blocks rather than fails open when an adapted project's config is absent, empty, or unparseable. Five paths (the persona-selection config plus Set B's four registration-surface literals) route through a human-confirmation branch instead of an outright deny: the gate emits `permissionDecision: "ask"`, never a grant, gated by a two-tier allowlist (Set A asks in `default\|plan\|acceptEdits\|auto`; Set B in `default\|plan\|auto`, excluding `acceptEdits`) that never emits `ask` from a subagent — this conditionality is self-referential, since the row's own subject is the gate's registration surface. U5: a Set B `asked` audit line with no paired `completed` line is ambiguous by design and does not distinguish a denial from an approval that later disabled the hook. |
