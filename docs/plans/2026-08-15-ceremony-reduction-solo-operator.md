...
**not** validate `.claude/persona-config.json` against the schema, and does
**not** check `agents/` ↔ `.claude/agents/` content parity — so Step 1 and
Step 5 assert those directly with `jq` rather than leaning on the suite.
...
- P2 "Prefer deterministic scripts over LLM re-derivation": satisfied —
  ...
  `node bin/cli.js --update` in Step 5. No step hand-edits `fileHashes` or
  any `.claude/agents/*.md` mirror.
...
- P5 "`tests/validate.sh` is the merge gate": satisfied — every step from 2
  onward carries `bash tests/validate.sh` exit 0 as a criterion, and Step 1
  adds a direct `jq`/`json.tool` assertion because the suite does not cover
...
- Any `.claude/` mirror — Step 5's job.
...
- **AC5.5** Attributable mirror content (independent of R4's in-flight work,
  which also moves AC5.4): `grep -c '≤5' .claude/agents/orchestrator.md` is
  ≥ 1 (baseline today `0`), **and** `grep -c '≤2-unit' .claude/persona-protocol.md`
  prints `0` (baseline today `1`).
