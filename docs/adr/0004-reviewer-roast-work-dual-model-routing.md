...
- **Cost claim superseded:** This ADR's rationale (issue context, line 37) states fable roast-work is "cheap relative to opus" to justify the separate advisory pass. That cost claim is now superseded by `templates/persona-protocol.md`'s statement that fable is the single most expensive model tier available to this system. The architectural rationale for a separate fable dispatch remains sound (preserving opus for the judgment-critical gate while using fable's bulk-context strength for non-gating critique), but the efficiency motivation stated here should be disregarded.
