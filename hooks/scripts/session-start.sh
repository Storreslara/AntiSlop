...
if [ -n "$adapted_version" ] && [ -n "$current_version" ] && [ "$adapted_version" != "$current_version" ]; then
  context_parts+=("seb-personas plugin is v${current_version} but this project was adapted at v${adapted_version} - run /seb-personas:setup-personas --update to resync.")
fi
