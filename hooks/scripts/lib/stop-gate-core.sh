...
marker_format_valid() {
  local unit="$1" verb="$2" first_line content marker_type
...
review_join_state() {
  ...
  for stamp in "${stamps[@]}"; do
    ...
    for pair in pass:PASS fail:FAIL; do
      ...
      if ! marker_format_valid "$unit" "$verb"; then
        continue
      fi
      case "$prior_mtime" in
        ''|*[!0-9]*)
...
if [ "$hook_event" = "SubagentStop" ] && [ "$(identity_persona_name "$agent_type")" = "reviewer" ]; then
  if persona_matches_grant "$agent_type" reviewer; then
    ...
    if [ "${#blocked_markers[@]}" -gt 0 ]; then
      state_append_audit_log "review-audit.log" "$(printf '%s verdict=blocked flags-kept' "$(date -u +%Y-%m-%dT%H:%M:%SZ)")"
    fi
    if [ "${#escalated_markers[@]}" -gt 0 ]; then
      state_append_audit_log "review-audit.log" "$(printf '%s verdict=escalated flags-kept' "$(date -u +%Y-%m-%dT%H:%M:%SZ)")"
    fi
    ...
    if [ "${JOIN_STAMP_COUNT:-0}" -eq 0 ]; then
      ...
      while [ "$idx" -lt "${#JOIN_SATISFIED_STAMPS[@]}" ]; do
        ...
        if [ "$mcc_mode" != off ]; then
          ...
          state_append_audit_log "review-audit.log" "$(printf '%s marker-commit-check=%s unit=%s' \
        ...
        state_append_audit_log "review-audit.log" "$(printf '%s join-consumed=%s' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      ...
      while [ "$idx" -lt "${#JOIN_UNSATISFIED_UNITS[@]}" ]; do
        state_append_audit_log "review-audit.log" "$(printf '%s cleared-by=reviewer marker=MISSING unit=%s' \
  ...
  if [[ $drift_out == *" config=drift "* ]]; then
    ...
    if [ -f "$override_file" ]; then
      ...
      case "$override_content" in
        "defer: "|"skip: ") ;;
        "defer: "*)
          state_append_audit_log "review-audit.log" "$(printf '%s config-drift-%s' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$override_content")"
          overridden=true
          ;;
        "skip: "*)
          state_append_audit_log "review-audit.log" "$(printf '%s config-drift-%s' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$override_content")"
...
if microworld_skip_ok "$project_dir" "${dot}/microworld-audit.log" "${baseline_sha:-}" "$moved"; then
  state_append_audit_log "review-audit.log" "$(printf '%s microworld-skip=testAndLintCommand' "$(date -u +%Y-%m-%dT%H:%M:%SZ)")"
  allow
fi
