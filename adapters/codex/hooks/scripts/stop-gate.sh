...
if [ "$hook_event" = "SubagentStop" ] && [ "$(identity_persona_name "$agent_type")" = "reviewer" ]; then
  if persona_matches_grant "$agent_type" reviewer; then
    ...
    escalated_markers=( "${project_dir}"/.codex/reviewed/*.escalated )
    shopt -u nullglob
    # Both globs log, so a blocked unit and an escalated one in the same turn
    # stay distinguishable in the audit log. `.directed` is deliberately not
    # globbed. allow() only after both, so neither token is skipped.
        ...
        if [ "$mcc_mode" != off ]; then
          ...
          if [ "$mcc_state" = mismatch ]; then
            ...
            if [ "$mcc_mode" = block ]; then
              echo "Remediation: correct the commit: field in .codex/reviewed/${unit}.pass to name the unit's own final commit, then re-run." >&2
              exit 2
            fi
          fi
        fi
