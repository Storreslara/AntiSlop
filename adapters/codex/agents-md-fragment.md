...
The `via:` line may be present, recording how the decision was composed and delivered:
absent (hand-typed, pre-existing default), `via: terminal` (terminal copy/heredoc path),
or `via: dashboard` (Microworld dashboard confirm-write path). When present, the reviewer
appends ` via: <value>` to the `human:` attestation line.
...
|---|---|---|---|
| **Approve** | `.pass`, with the `commit:` field copied verbatim from the standing `.escalated` marker's own `commit:` line (never re-derived), plus an appended `human: approved by <name> <UTC ISO-8601> examples: <token>` attestation **with optional ` via: <value>` appended if present in the file**, quoting the file | - | unit done |
